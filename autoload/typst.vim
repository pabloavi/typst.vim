function! typst#TypstWatch(...)
  ...
  let l:file_to_watch = expand('%') 
  " if not main.typ or master.typ, search for them in the current directory
  if l:file_to_watch !~? 'main.typ' && l:file_to_watch !~? 'master.typ'
    let l:file_to_watch = findfile('master.typ', '.;')
    if empty(l:file_to_watch)
      let l:file_to_watch = findfile('main.typ', '.;')
    endif
  endif
  let l:cmd = g:typst_cmd
        \ . ' ' . join(a:000)
        \ . ' watch'
        \ . ' --root ' . substitute(fnamemodify(finddir('.git', '.;'), ':p:h'), '/.git$', '', '')
        \ . ' --diagnostic-format short'
        \ . " '" . l:file_to_watch . "'"

  if !g:typst_no_editor
...
function! typst#in_math(...) abort
  let l:stack = call('typst#synstack', a:000)
  " check if -1 will retrieve error out of bounds
  if len(l:stack) == 0
    return 0
  endif
